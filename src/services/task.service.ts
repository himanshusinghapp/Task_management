...
export class TaskService {
  ...
  async updateTask(taskId: string, userId: string, role: string, data: UpdateTaskDto) {
    try {
      ...
      if (role !== 'user' && value.assignedTo) {
        logServiceMethod('TaskService', 'updateTask', LOGGER_MESSAGES.USER_CREATION_FAILED, { taskId, userId, role });
        throw Exceptions.Forbidden(USER_MESSAGES.ADMIN_ONLY_ASSIGN);
      }
  ...
  async uploadAttachments(taskId: string, files: string[], role: string, userId: string) {
    try {
      ...
      if (role !== 'user' && assignedToId !== userId) {
        logServiceMethod('TaskService', 'uploadAttachments', LOGGER_MESSAGES.TASK_FETCH_FAILED, { taskId, userId, role, assignedToId });
        throw Exceptions.Forbidden(USER_MESSAGES.ACCESS_DENIED);
      }
